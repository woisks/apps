...
class JwtService
{
     ...
     * @return string
     */
    public static function jwt_parser_token(string $token_name = null): string
    {

        $token_name        = $token_name ?? config('woisk.jwt.token_name');
        $token_cookie_name = $token_cookie_name ?? config('woisk.jwt.token_cookie_name');
        ...
        return (string) request()->input($token_name) ?? (string) request()->cookie($token_cookie_name);
    }
