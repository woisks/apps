...
class CaptchaRepository
{
    ...
    {
        return self::$model->where('code', $code)
            ->where('name', $name)
            ->where('status', 0)
            ->first();
    }
     ...
     * @return mixed
     */
    public function created(string $name, int $code, int $minute = null)
    {
        $expire_time = $minute ?? config('woisk.captcha.expire_time');
        ...
        return self::$model->create([
            ...
            'expire_time' => Carbon::now()->addMinute($expire_time)->timestamp
        ]);
    }
